...
| `/new` | **New ticket** | analyst, admin (blocked by URL for others, like every restricted page). Every optional field says **Automatic · AI reads it / rules compute it** and has a method tag with (i); staff values are kept and cross-checked | Required: summary, description, reporter. Optional "Set details yourself" for every AI-fillable field. Buttons: *Create and watch AI triage* / *Create only* |
