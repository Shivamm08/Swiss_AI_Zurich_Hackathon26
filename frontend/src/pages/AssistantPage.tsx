  ...
  return (
    <div className="flex max-w-3xl flex-col gap-5">
      <PageHeader title="Assistant"
        subtitle={<>Ask about past resolutions and services. Answers are grounded in the knowledge base and cite their sources.{ticketId && ' This question is scoped to the selected ticket.'}</>} />
      <form
          ...
          <Card title="Sources">
            <ul className="flex flex-col gap-2 text-sm">
              {ask.data.citations.map((c) => (
                <li key={c.ref_id}>
                  <span className="font-mono text-xs text-muted">[{c.ref_id}]</span> {c.title}
                </li>
              ))}
            </ul>
          </Card>
