...
def get_workload(team: str = Query(..., description="Team name, e.g. 'Client Services'"), db: Session = Depends(get_db)) -> Workload:
    ...
    approved = db.execute(
        select(Ticket.assignee).join(Review, Review.ticket_id == Ticket.id)
        .where(Review.action.in_(("approve", "edit")), Review.created_at >= week_ago)
    ).scalars().all()

    rows = []
    for m in members:
        mine = [t for t in active if t.assignee == m.email]
        rows.append(WorkloadMember(
            ...
            approved_7d=approved.count(m.email),
        ))
